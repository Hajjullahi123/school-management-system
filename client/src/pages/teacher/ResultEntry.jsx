    ...
    <div className="space-y-6 relative">
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl shadow-lg ring-1 ring-black ring-opacity-5 overflow-hidden border border-gray-100">
          ...
          {/* Bottom Save Button */}
          <div className="p-4 sm:p-6 border-t border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-4 bg-gray-50/80 rounded-b-xl">
            <p className="text-sm text-gray-500 font-medium">Please verify all scores before saving.</p>
            <button
              onClick={handleSave}
              disabled={saving || students.length === 0 || isDemo}
              className={`${isDemo ? 'bg-gray-400 cursor-not-allowed' : 'bg-green-600 hover:bg-green-700'} text-white px-8 py-3 rounded-lg disabled:opacity-50 flex items-center font-bold shadow-md transition-all active:scale-95`}
            >
              {saving ? (
                <>
                ...
                <>
                  ...
                  {isDemo ? 'Save Restricted (Demo)' : 'Save All Results'}
                </>
              )}
            </button>
          </div>
