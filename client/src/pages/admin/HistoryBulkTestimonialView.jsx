...
const HistoryBulkTestimonialView = () => {
  ...
  const fetchBulkData = async () => {
    try {
      setLoading(true);
      const response = await api.get(`/api/testimonials/bulk-history/${classId}/${sessionId}`);
      if (!response.ok) {
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: `Bulk_History_Testimonials_${classId}_${sessionId}`,
    pageStyle: `
  ...
  if (testimonials.length === 0) return (
    <div className="p-8 max-w-2xl mx-auto">
      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6 text-center">
        ...
        <p className="text-yellow-600 mb-6">No testimonials found for this class in the selected session.</p>
      ...
      <div className="bg-white border-b p-4 sticky top-0 z-50 no-print shadow-sm">
        <div className="container mx-auto flex justify-between items-center">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="font-bold text-lg">Bulk Testimonials from Promotion History</h1>
...
export default HistoryBulkTestimonialView;
