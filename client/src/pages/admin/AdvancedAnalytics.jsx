...
const AdvancedAnalytics = () => {
  ...
  const [diagnosingStudentId, setDiagnosingStudentId] = useState(null);
  ...
  const handleAIDiagnosis = async (student) => {
    setDiagnosingStudentId(student.studentId);
    setIsDiagnosing(true);
    try {
      const termId = getTermId();
      const response = await api.post(`/api/advanced-analytics/ai/student-diagnosis/${student.studentId}`, {
        termId
      });
      ...
      if (response.ok) {
        ...
        toast.error(err.message || 'Failed to generate AI diagnosis');
      }
    } catch (error) {
                    ...
                    <div key={idx} className={`bg-white border-l-4 rounded-lg shadow-md p-6 ${student.riskLevel === 'High' ? 'border-red-500' :
                      ...
                      <div className="flex gap-2 mt-3">
                        <button
                          ...
                          className="flex-1 px-4 py-2 bg-blue-50 text-blue-600 rounded-xl hover:bg-blue-100 text-xs font-bold transition-all border border-blue-100"
                        >
                          Log Intervention
                        </button>
                        <button
                          onClick={() => handleAIDiagnosis(student)}
                          disabled={isDiagnosing && diagnosingStudentId === student.studentId}
                          className="flex-1 px-4 py-2 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 text-xs font-bold transition-all shadow-md shadow-indigo-100 flex items-center justify-center gap-2 disabled:opacity-70"
                        >
                          {isDiagnosing && diagnosingStudentId === student.studentId ? (
          ...
          {showDiagnosisModal && selectedDiagnosis && (
            <div className="fixed inset-0 z-[110] bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-4">
              <div className="bg-white w-full max-w-4xl max-h-[90vh] rounded-[32px] shadow-2xl overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
                <div className="bg-gradient-to-br from-indigo-600 to-purple-700 p-8 text-white shrink-0">
                  <div className="flex justify-between items-start">
                    <div>
                      <h3 className="text-2xl font-black italic tracking-tight uppercase mb-1">Deep Academic Diagnosis</h3>
                      <p className="text-indigo-100 text-xs font-bold uppercase tracking-widest flex items-center gap-2">
                        <span className="w-2 h-2 bg-green-400 rounded-full animate-pulse"></span>
                        AI Report for {selectedDiagnosis.studentName}
                      </p>
                    </div>
                    <button 
                ...
                <div className="p-8 overflow-y-auto prose prose-slate prose-indigo max-w-none">
                  <ReactMarkdown 
                    components={{
                      h1: ({node, ...props}) => <h1 className="text-2xl font-black text-slate-800 border-b pb-2 mb-4 uppercase tracking-tighter italic" {...props} />,
                      ...
                      p: ({node, ...props}) => <p className="text-slate-600 leading-relaxed text-sm mb-4" {...props} />,
                      ul: ({node, ...props}) => <ul className="list-disc pl-5 space-y-2 text-sm text-slate-600 mb-4" {...props} />,
                      li: ({node, ...props}) => <li className="marker:text-indigo-500 font-medium" {...props} />,
                      strong: ({node, ...props}) => <strong className="text-slate-900 font-bold" {...props} />
                    }}
                  >
                    {selectedDiagnosis.content}
                  </ReactMarkdown>
                </div>
                ...
                <div className="p-6 bg-slate-50 border-t flex justify-end shrink-0">
                  <button 
                    onClick={() => setShowDiagnosisModal(false)}
                    className="px-8 py-3 bg-slate-200 text-slate-700 rounded-2xl font-black uppercase text-xs hover:bg-slate-300 transition-colors"
                  >
                    Close Report
                  </button>
                </div>
