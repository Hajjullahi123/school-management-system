  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      ...
      <div className="mt-12 group">
        <div className="bg-white rounded-[24px] shadow-2xl overflow-hidden border border-slate-100 relative">
          <div className="absolute top-0 left-0 w-1 h-full bg-gradient-to-b from-primary via-orange-500 to-indigo-600"></div>
          <div className="px-8 py-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
            <div>
              <h2 className="text-xl font-black text-gray-900 tracking-tight uppercase italic">Ancillary Obligations</h2>
              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.2em]">Miscellaneous Service Items</p>
            </div>
            <div className="flex items-center gap-3">
              <span className="text-[9px] bg-slate-200 text-slate-600 px-3 py-1.5 rounded-full font-black uppercase tracking-[0.15em]">Sessional</span>
          ...
          <div className="overflow-x-auto">
            {miscFees.length === 0 ? (
              <div className="p-12 text-center">
                ...
                <p className="text-slate-400 font-bold uppercase tracking-widest text-xs">No pending ancillary records</p>
              </div>
            ) : (
              <table className="min-w-full divide-y divide-slate-100">
                <thead>
                  <tr className="bg-slate-50/30">
                    <th className="px-8 py-5 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest">Provision Description</th>
                    <th className="px-8 py-5 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest">Amount</th>
                    <th className="px-8 py-5 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest">Ledger Status</th>
                  ...
                  {miscFees.map((fee) => {
                    ...
                    return (
                      <React.Fragment key={fee.id}>
                        <tr className="hover:bg-slate-50/80 transition-all cursor-default group/row">
                          <td className="px-8 py-6">
                            <div className="text-sm font-black text-slate-900 group-hover/row:text-primary transition-colors">{fee.title}</div>
                            {fee.description && (
                              <div className="text-[10px] text-slate-500 mt-1 font-medium">{fee.description}</div>
                            )}
                          </td>
                          <td className="px-8 py-6 whitespace-nowrap">
                            <div className="text-base font-black text-slate-900 tracking-tight">₦{formatNumber(fee.amount)}</div>
                          </td>
                          <td className="px-8 py-6 whitespace-nowrap">
                            {isFullyPaid ? (
                              <div className="flex items-center gap-2">
                                <span className="w-2 h-2 rounded-full bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]"></span>
                                <span className="text-[10px] font-black uppercase tracking-[0.15em] text-emerald-600">Settled</span>
                              </div>
                            ) : paidAmount > 0 ? (
                              <div className="flex flex-col gap-1">
                                <div className="flex items-center gap-2">
                                  <span className="w-2 h-2 rounded-full bg-amber-500 animate-pulse"></span>
                                  <span className="text-[10px] font-black uppercase tracking-[0.15em] text-amber-600">Partial: ₦{formatNumber(paidAmount)}</span>
                                </div>
                                <div className="w-24 h-1 bg-slate-100 rounded-full overflow-hidden">
                                  <div className="h-full bg-amber-500" style={{width: `${(paidAmount/fee.amount)*100}%`}}></div>
                                </div>
                              </div>
                            ) : (
                              <div className="flex items-center gap-2">
                                <span className="w-2 h-2 rounded-full bg-red-400"></span>
                                <span className="text-[10px] font-black uppercase tracking-[0.15em] text-red-500">Unremitted</span>
                              </div>
                            )}
                          </td>
                          <td className="px-8 py-6 whitespace-nowrap text-right">
                            {!isFullyPaid && (
                              <div className="inline-flex items-center gap-1 text-[9px] text-slate-400 font-bold uppercase italic tracking-tighter bg-slate-50 px-2 py-1 rounded">
                        ...
                        {fee.payments && fee.payments.map((p) => (
                          <tr key={p.id} className="bg-slate-50/40 opacity-80 border-l-2 border-emerald-400/30">
                            <td colSpan="2" className="px-8 py-3 text-[11px] text-slate-500 pl-16">
                              <span className="flex items-center gap-3">
                                <div className="w-1.5 h-1.5 bg-emerald-400 rounded-full shadow-[0_0_5px_rgba(52,211,153,0.5)]"></div>
                                Digital Ingress via <span className="font-bold text-slate-700">{p.paymentMethod}</span> — {formatDate(p.paymentDate)}
                              </span>
                            </td>
                            <td className="px-8 py-3 text-xs font-black text-emerald-600">
                            ...
                            <td className="px-8 py-3 text-right">
                              <button
                                ...
                                className="group/btn bg-white hover:bg-primary border border-slate-200 hover:border-primary text-slate-600 hover:text-white py-1.5 px-3 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all shadow-sm flex items-center gap-2 ml-auto"
                              >
                                <span>Generate Digital Receipt</span>
                                <svg className="w-3 h-3 opacity-50 group-hover/btn:translate-x-0.5 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z" /></svg>
                              </button>
                            </td>
                          </tr>
                        ))}
