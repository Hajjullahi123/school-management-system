...
import { saveBlobAsFile } from '../../utils/mobileDownload';
...
const TermReportCard = () => {
  ...
  const handleDownloadPDF = async () => {
    ...
    setDownloading(true);
    setPdfProgress(5);
    setPdfProgressLabel('Preparing report cards...');
    ...
    try {
      ...
      // Temporarily normalize transforms so html2canvas renders exact A4 proportions
      ...
      for (let i = 0; i < total; i++) {
        ...
        setPdfProgress(Math.round(5 + ((i) / total) * 85));
        setPdfProgressLabel(`Capturing report ${i + 1} of ${total}...`);

        // Yield for responsive UI
              ...
              <div key={idx} className="mb-20 last:mb-0 print:mb-0">
                ...
                <div className="report-card-mobile-wrapper overflow-x-auto pb-8 print:overflow-visible">
                  <div className="report-card-scaler origin-top-left sm:origin-top scale-[0.45] xs:scale-[0.55] sm:scale-100 transition-transform duration-500 print:scale-100 print:transform-none">
                    <div key={idx} className={`relative bg-white p-8 print:p-0 my-8 print:my-0 shadow-2xl print:shadow-none text-black ${borderStyle} emerald-print-A4 mx-auto w-[210mm] min-w-[210mm]`} style={{ fontFamily: reportFont, borderColor: layout !== 'minimal' ? reportColor : undefined, pageBreakAfter: 'always', breakAfter: 'page' }}>
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-4 mt-12 mb-8 print:hidden">
            ...
            {reportData && user?.role !== 'student' && user?.role !== 'parent' && (
              <button
                ...
                className="group/btn bg-[#25D366] text-white px-8 py-4 rounded-[24px] font-black uppercase tracking-widest text-xs shadow-xl hover:scale-105 active:scale-95 transition-all flex items-center gap-3"
              >
