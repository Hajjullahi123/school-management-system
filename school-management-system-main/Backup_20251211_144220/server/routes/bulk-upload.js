    ...
    for (const studentData of students) {
      try {
        ...
        const classInfo = await prisma.class.findUnique({
          where: { id: parseInt(studentData.classId) }
        });
        ...
        const admissionNumber = await getUniqueAdmissionNumber(prisma, baseAdmissionNumber, parseInt(studentData.classId));
    ...
    for (const resultData of results) {
      try {
        ...
        const student = await prisma.student.findUnique({
          where: { admissionNumber: resultData.admissionNumber },
          include: { user: true }
        });
