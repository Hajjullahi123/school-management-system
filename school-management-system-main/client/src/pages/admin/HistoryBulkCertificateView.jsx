...
const HistoryBulkCertificateView = () => {
  ...
  const fetchBulkData = async () => {
    try {
      setLoading(true);
      const response = await api.get(`/api/certificates/bulk-history/${classId}/${sessionId}`);
      if (!response.ok) {
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: `Bulk_History_Certificates_${classId}_${sessionId}`,
    pageStyle: `
            @page {
                size: A4 landscape;
                margin: 0;
  ...
  if (certificates.length === 0) return (
    <div className="p-8 max-w-2xl mx-auto">
      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6 text-center">
        ...
        <p className="text-yellow-600 mb-6">No certificates found for this class in the selected session.</p>
    ...
    <div className="bg-gray-100 min-h-screen">
      ...
      <div className="bg-white border-b p-4 sticky top-0 z-50 no-print shadow-sm">
        <div className="container mx-auto flex justify-between items-center">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="font-bold text-lg">Bulk Certificates from Promotion History</h1>
      ...
      <div ref={componentRef} className="print-container">
        {certificates.map((cert) => {
          ...
          return (
            <div key={cert.id} className="certificate-page bg-white p-0 m-0 relative emerald-print-A4-landscape mx-auto my-4 shadow-xl md:shadow-none" style={{ width: '297mm', minWidth: '297mm', height: '210mm', overflow: 'hidden', fontFamily: certFont }}>
...
export default HistoryBulkCertificateView;
