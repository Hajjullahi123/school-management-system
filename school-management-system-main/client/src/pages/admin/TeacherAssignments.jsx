...
const TeacherAssignments = () => {
  ...
  const groupedAssignments = (Array.isArray(assignments) ? assignments : []).reduce((acc, assignment) => {
    if (groupBy === 'teacher') {
      const teacherName = assignment.teacher ? [assignment.teacher.firstName, assignment.teacher.lastName, assignment.teacher.middleName].filter(Boolean).join(' ') : 'Unidentified Teacher';
      if (!acc[teacherName]) {
        acc[teacherName] = {
          title: teacherName,
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                >
                  ...
                  {[...teachers].sort((a, b) => [a.firstName, a.lastName, a.middleName].filter(Boolean).join(' ').localeCompare([b.firstName, b.lastName, b.middleName].filter(Boolean).join(' '))).map((teacher) => (
                    <option key={teacher.id} value={teacher.id}>
                      {[teacher.firstName, teacher.lastName, teacher.middleName].filter(Boolean).join(' ')}
                    </option>
                  ))}
                </select>
              </div>
          ...
          <div key={key} className="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div className="p-4">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                {data.assignments.sort((a, b) => (a.subject?.name || '').localeCompare(b.subject?.name || '')).map((assignment) => (
                  <div
                    key={assignment.id}
                    ...
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        {groupBy === 'teacher' ? (
                          ...
                          <>
                            <p className="font-medium text-gray-900">{assignment.subject?.name || 'Unknown Subject'}</p>
                            <p className="text-sm text-gray-600">Tr. {[assignment.teacher?.firstName, assignment.teacher?.lastName, assignment.teacher?.middleName].filter(Boolean).join(' ') || 'Unidentified'}</p>
                          </>
                        )}
                      </div>
                      <div className="flex">
