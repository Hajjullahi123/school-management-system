 ...
 */
async function getUniqueAdmissionNumber(prisma, baseAdmissionNumber, classId) {
  // Convert classId to integer if it's provided and valid
  // Handle empty strings, null, undefined, and invalid numbers
  const parsedClassId = classId ? parseInt(classId) : null;
  const classIdInt = parsedClassId && !isNaN(parsedClassId) ? parsedClassId : null;

  // Count existing students in this class to determine the next number
  ...
  // Check if admission number exists (in case of gaps from deletions)
  while (true) {
